"""
Solving: https://adventofcode.com/2024/day/5#part2
"""
...
def to_ordered(array: list[int], rules: dict[int, set[int]]) -> list[int]:
    ...
    for i in range(len(array)):
        ...
        for j in range(len(ordered_array)):
            if ordered_array[j] in rules.get(next, set()):
                ordered = False
                ordered_array.insert(j, next)
                break

        if ordered:
            ordered_array.append(next)
...
def solve(data: str) -> int:
    ...
    for update in updates:
        ...
        if not ordered:
            ordered_update = to_ordered(update, after_rules)

            res += ordered_update[(len(ordered_update) - 1) // 2]
